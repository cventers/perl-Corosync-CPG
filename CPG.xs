...
HV *
cpgc_cpg_address_unpack (const struct cpg_address *address)
{
    dTHX;
    HV *hv;
    hv = newHV();
              ...
              size_t msg_len)
{
    ...
    /* add arguments */
    XPUSHs((SV*)newRV((SV*)self));
    XPUSHs((SV*)newSVpv(group_name->value, group_name->length));
    XPUSHs((SV*)newSVpv(msg, msg_len));
...
)
{
    ...
    for (i = 0; i < member_list_size; i++) {
        hv = cpgc_cpg_address_unpack(&member_list[i]);
              ...
              size_t joined_list_entries)
{
    ...
    mlav = cpgc_cpg_address_list_unpack(member_list, member_list_entries);
    llav = cpgc_cpg_address_list_unpack(left_list, left_list_entries);
    jlav = cpgc_cpg_address_list_unpack(joined_list, joined_list_entries);
    ...
    /* add arguments */
    XPUSHs(newRV((SV*)self));
    XPUSHs((SV*)newSVpv(group_name->value, group_name->length));
    XPUSHs(newRV((SV*)mlav));
    XPUSHs(newRV((SV*)llav));
    XPUSHs(newRV((SV*)jlav));
...
cpgc__membership_get(self, name)
	...
	av = cpgc_cpg_address_list_unpack(member_list, member_list_size);
